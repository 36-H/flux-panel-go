// ==========================================================================
// Code generated and maintained by GoFrame CLI tool. DO NOT EDIT.
// ==========================================================================

package internal

import (
	"context"

	"github.com/gogf/gf/v2/database/gdb"
	"github.com/gogf/gf/v2/frame/g"
)

// NodeDao is the data access object for the table node.
type NodeDao struct {
	table    string             // table is the underlying table name of the DAO.
	group    string             // group is the database configuration group name of the current DAO.
	columns  NodeColumns        // columns contains all the column names of Table for convenient usage.
	handlers []gdb.ModelHandler // handlers for customized model modification.
}

// NodeColumns defines and stores column names for the table node.
type NodeColumns struct {
	Id          string //
	Name        string //
	Secret      string //
	Ip          string //
	ServerIp    string //
	PortSta     string //
	PortEnd     string //
	Version     string //
	Http        string //
	Tls         string //
	Socks       string //
	CreatedTime string //
	UpdatedTime string //
	Status      string //
}

// nodeColumns holds the columns for the table node.
var nodeColumns = NodeColumns{
	Id:          "id",
	Name:        "name",
	Secret:      "secret",
	Ip:          "ip",
	ServerIp:    "server_ip",
	PortSta:     "port_sta",
	PortEnd:     "port_end",
	Version:     "version",
	Http:        "http",
	Tls:         "tls",
	Socks:       "socks",
	CreatedTime: "created_time",
	UpdatedTime: "updated_time",
	Status:      "status",
}

// NewNodeDao creates and returns a new DAO object for table data access.
func NewNodeDao(handlers ...gdb.ModelHandler) *NodeDao {
	return &NodeDao{
		group:    "default",
		table:    "node",
		columns:  nodeColumns,
		handlers: handlers,
	}
}

// DB retrieves and returns the underlying raw database management object of the current DAO.
func (dao *NodeDao) DB() gdb.DB {
	return g.DB(dao.group)
}

// Table returns the table name of the current DAO.
func (dao *NodeDao) Table() string {
	return dao.table
}

// Columns returns all column names of the current DAO.
func (dao *NodeDao) Columns() NodeColumns {
	return dao.columns
}

// Group returns the database configuration group name of the current DAO.
func (dao *NodeDao) Group() string {
	return dao.group
}

// Ctx creates and returns a Model for the current DAO. It automatically sets the context for the current operation.
func (dao *NodeDao) Ctx(ctx context.Context) *gdb.Model {
	model := dao.DB().Model(dao.table)
	for _, handler := range dao.handlers {
		model = handler(model)
	}
	return model.Safe().Ctx(ctx)
}

// Transaction wraps the transaction logic using function f.
// It rolls back the transaction and returns the error if function f returns a non-nil error.
// It commits the transaction and returns nil if function f returns nil.
//
// Note: Do not commit or roll back the transaction in function f,
// as it is automatically handled by this function.
func (dao *NodeDao) Transaction(ctx context.Context, f func(ctx context.Context, tx gdb.TX) error) (err error) {
	return dao.Ctx(ctx).Transaction(ctx, f)
}
